...
void handle_shutdown(){
  ...
  ret = write(sockfd,write_buf,strlen(write_buf));
  if (ret < 0){
    temp_errno = errno;
    error_handle("write() to socket ID number",temp_errno);
  }
...
//parsing options
  ...
  if (server == NULL){
    fprintf(stderr, "No such host: %s",hostname);
    exit(1);
  }
  ...
  while(run_flag){
    ...
    if (generating && (cur_clock.tv_sec >= next_time)){
      ...
      if (ret < 0){
        temp_errno = errno;
        error_handle("write() to socket temperature", temp_errno);
      }
    ...
    if (ret == -1){
      ...
      num_read = read(sockfd, cmd_buf, 2048);
      if (num_read == 0){
        //EOF received
        handle_shutdown();
      }
